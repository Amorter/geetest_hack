...
function findClosestValue(trace_list, target) {
	...
	while (left <= right) {
		...
		// 更新最接近的值
		if (Math.abs(trace_list[mid][trace_list[mid].length - 1][0] - target) < Math.abs(closest[closest.length - 1][0] - target)) {
		...
		} else if (trace_list[mid][trace_list[mid].length - 1][0] < target) {
			left = mid + 1;
		} else {
			right = mid - 1;
		}
